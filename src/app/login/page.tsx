...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Update client-side context immediately with data returned by login route
    if (payload?.user) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!regRes.ok) {
      setRegError(regPayload?.error ?? "Registration failed. Please try again.");
      setRegLoading(false);
      return;
    }
    ...
    if (!loginRes.ok) {
      // Registration succeeded but auto-login failed — redirect to login tab
